...
<html>
    <head>
        ...
        <script>
            function confirm_delete(id)
            {
                ...
                if (r == true)
                {
                    window.open("transport_delete.php?id="+id, "_self");
                }
                else
                {
    ...
    <body>
            ...
            <form action="transport_search.php" method="POST">
            ...
            <?php
                if(isset($_POST['btn']))
                {
                    ...
                    $transport = searchtransport(trim($_POST['text_to_search']),"name");
                    //print_r($transport);
                    if(count($transport) > 0)
                    {
                        echo "<table border='1' style= 'border-collapse: collapse;'>";
                            echo "<tr>";
                                $keys = array_keys ($transport[0]);
                            ...
                            for($i =0 ;$i < count($transport ) ; $i++)
                            {
                                ...
                                {
                                    $key = $keys[$j];
                                    echo "<td >".$transport[$i][$key]."</td>";
                                }
                                $id = $transport[$i]['id'];
                                
                                echo "<td >"."<a href = 'transport_insert_form.php?action=edit&id=$id' >แก้ไข </a>"."</td>";
                                echo "<td >".
